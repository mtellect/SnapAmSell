...
class _ShowViewedMyProfileState extends State<ShowViewedMyProfile>
  ...
  loadPeople() async {
    QuerySnapshot shots = await Firestore.instance
        ...
        .where(VIEWED_LIST, arrayContains: userModel.getObjectId())
        .orderBy(TIME_UPDATED, descending: true)
        .startAt([
          peopleList.isEmpty
              ? DateTime.now().millisecondsSinceEpoch
              : ((peopleList[peopleList.length - 1].getInt(TIME_UPDATED)))
        ])
    ...
    for (DocumentSnapshot doc in shots.documents) {
      ...
      int index = peopleList
          .indexWhere((model) => model.getObjectId() == bm.getObjectId());
      if (index == -1) {
        peopleList.add(bm);
      }
    }
          ...
          new Expanded(
            ...
            child: !setup
                ...
                : setup && peopleList.isEmpty
                    ? emptyLayout(Icons.person, "Nothing to display", "",
                        trans: true)
                    : Container(
//                        color: white,
